...
{
    $result = $link->query("SELECT COUNT(num_commande) FROM commande WHERE etat_commande = 1");
    if ($result === false)
    {
    ...
    else
    {
        ...
        {
            $order_id = array();
            $result = $link->query("SELECT num_commande FROM commande WHERE etat_commande = 1 ORDER BY date_commande ASC");
